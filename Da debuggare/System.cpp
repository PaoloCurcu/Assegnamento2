...
Warehouse::Warehouse() :
		month { 0 }, cash_desk { 0 }, cash_available { 0 } {
}

Warehouse::Warehouse(double initCash_desk) :
		month { 0 }, cash_desk { initCash_desk }, cash_available { initCash_desk } {
	fill_data();
}
...
//AcceptOrder mi dice se ho soldi a sufficienza per effettuare l'ordine
bool Warehouse::acceptOrder(Order ord) {
...
void Warehouse::check_situation() {
	//if (orderTemp.size() > 0) {
		while (acceptOrder(orderTemp[0])&&orderTemp.size() > 0) {
...
void Warehouse::printAll() {
	...
	for (Order p : orderReady) {
		cout << "- " << p.quantityOrdered() << " " << p.modelName()
				<< " del mese " << p.timeStamp() << endl;
	}
