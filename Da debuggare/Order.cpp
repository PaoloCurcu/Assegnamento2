...
Order::Order(orderInit fMain, const vector<vector<components> > &models_infos,
	...
	time_stamp { fMain.time_stamp }, model_id { fMain.model_id }, quantity_ordered {
	                                                                                fMain.quantity_ordered }, ready { false } {
	for (auto mod : models_infos) { //scorro il vettore con le info sui modelli
		if (model_id == mod[0].id) {
			model_name = mod[0].name;
			price = mod[0].price[0] * quantity_ordered;
			...
			for (unsigned int i = 1; i < mod.size(); i++) { //scorro i componenti all'interno del file saltando il primo
				...
				for (auto info : components_infos) { //scoroo il vettore con le informazioni dei componenti
					...
					if (mod[i].id == info.component_id) {
						...
						for(unsigned int k = 0; k<info.price.size(); k++) {
							mod[i].price.push_back(info.price[k]); //qui il problema
						}
					}
				}
			...
			//questo perch� mi serve come riferimento. Cos� so dove trovarlo qualunque sia il modello.
			components tmp;
			int indexMax = 0;
...
bool Order::updates() {
	...
	for (unsigned int i = 0; i < orderComponents.size(); i++) {
		if (orderComponents[i].status == 1) {
			if (orderComponents[i].monthsLeft == 1) {
...
void Order::fillComponentToBuy(const components &order,
                               vector<priceAssociation> &toFill) {
	bool findID = false;
	...
	if (!findID) {
		toFill.push_back(
			priceAssociation { order.id, order.quantity, order.price });
	}
}
...
//ritorna la somma di denaro che un determinato ordine necessita
double Order::orderValue() {
    double somma_totale = 0;
    for (unsigned int i=0; i<orderComponents.size(); ++i)
        somma_totale = (orderComponents[i].price[0]*orderComponents[i].quantity) + somma_totale;
    return somma_totale;
}
